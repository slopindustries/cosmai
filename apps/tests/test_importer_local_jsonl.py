...
Copy-adapted from ``experiments/integrated-p0/tests/test_importer_local_jsonl.py`` (M4).
Fixture names follow this tree's convention (``test_addon_capabilities.py``'s own docstring
states it): ``job_store`` for P0's ``store``, ``job_connection`` for ``connection``, and
``domain_store`` (already built in ``apps/tests/conftest.py``) for ``domain``. Everything else
— every scenario, every assertion, every docstring paragraph below — is the P0 original.
...
def payloads_of(connection: psycopg.Connection[Any]) -> set[bytes]:
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "select i.payload from cosmai.raw_item i "
            "join cosmai.raw_envelope e on e.id = i.envelope_id where e.source_id = %s",
            (SOURCE,),
        )
...
class TestAnImporterRuns:
    def test_every_row_becomes_one_raw_item(
        ...
        register(domain_store, dataset(tmp_path, *GOOD_ROWS))
    ...
    def test_the_payload_is_the_line_as_it_was_read(
        ...
        run_import(tmp_path, job_store, domain_store)

        assert payloads_of(job_connection) == {line.encode("utf-8") for line in GOOD_ROWS}
    ...
    def test_the_envelope_holds_the_whole_file(
        ...
        summary = domain_store.raw_summary(SOURCE)
    ...
    def test_the_cursor_records_where_it_stopped(
        ...
        assert domain_store.read_cursor(SOURCE, "rows") == {"lines_read": 2}
...
class TestMalformedAndPartiallyInvalidRows:
    ...
    def test_a_malformed_line_is_skipped_and_the_good_rows_survive(
        ...
        register(domain_store, dataset(tmp_path, GOOD_ROWS[0], "{not json at all", GOOD_ROWS[1]))
    ...
    def test_a_line_that_is_valid_json_but_not_a_record_is_skipped(
        ...
        register(domain_store, dataset(tmp_path, GOOD_ROWS[0], "[1, 2, 3]", '"a string"'))

        outcome = run_import(tmp_path, job_store, domain_store)

        assert outcome.accepted, outcome
        assert domain_store.count_items(SOURCE) == 1
    ...
    def test_a_row_missing_the_key_field_is_skipped(
        ...
        register(domain_store, dataset(tmp_path, GOOD_ROWS[0], '{"title": "no id here"}'))
    ...
    def test_a_file_of_nothing_but_bad_rows_succeeds_with_no_items(
        ...
        register(domain_store, dataset(tmp_path, "{oops", "[1]", '{"no": "key"}'))

        outcome = run_import(tmp_path, job_store, domain_store)

        assert outcome.accepted, outcome
        assert domain_store.count_items(SOURCE) == 0
        assert domain_store.raw_summary(SOURCE)["envelope_count"] == 1
    ...
    def test_blank_lines_are_not_counted_as_malformed(
        ...
        pass them all."""
        register(domain_store, dataset(tmp_path, GOOD_ROWS[0], "", "   ", GOOD_ROWS[1]))
...
class TestNothingOutsideTheApprovedRootIsRead:
    def test_a_member_escaping_the_root_fails_the_run(
        ...
        register(domain_store, approved, member="../secret.jsonl")
...
class TestTheImportersCopyOfTheCursorRulesIsChecked:
    ...
    def test_a_cursor_value_of_none_is_refused(
        ...
        outcome = self._run_synthetic(tmp_path, job_store, domain_store, """
...
""")
        ...
        assert outcome.error is not None
        assert domain_store.read_cursor(SOURCE, "rows") is None
...
""")
        ...
        assert outcome.error is not None
        assert domain_store.read_cursor(SOURCE, "elsewhere") is None
...
class TestConfigurationIsChecked:
    def test_a_missing_key_field_is_a_configuration_failure(
        ...
        register(domain_store, dataset(tmp_path, *GOOD_ROWS), config={})
