...
Copy-adapted from ``experiments/integrated-p0/tests/test_domain_store.py`` (M2 batch
2b). Only fixture names and schema-qualification changed: ``domain``/``store``/
``connection`` become ``domain_store``/``job_store``/``job_connection`` (this tree's
own naming, ``apps/tests/conftest.py``), the local ``domain`` fixture is gone
(``domain_store`` lives in ``conftest.py`` instead, sharing ``job_connection`` with
``job_store`` for the same reason P0's did — the atomicity tests need both stores on
one connection), and every ad-hoc SQL string is qualified ``cosmai.<table>`` (DP-032
D1/D3, the same adaptation ``apps/domain/store.py`` itself makes). No test's actual
assertion changed.
"""
...
def claim_one(job_store: JobStore, handler: str = "addon:collector.demo") -> tuple[UUID, UUID]:
    """Create and claim one job, returning ``(job_id, attempt_id)``."""
    job_id = job_store.create_job(handler, {"source_id": "demo"}, max_attempts=3)
    claimed = job_store.claim_next("worker-1", lease_seconds=60)
...
class TestSourceRegistry:
    def test_a_registered_source_reads_back_as_it_was_written(
        self, domain_store: DomainStore
    ) -> None:
        domain_store.register_source(a_source())
        row = domain_store.read_source("demo")
    ...
    def test_an_unregistered_source_reads_as_absent_rather_than_raising(
        ...
        assert domain_store.read_source("never-registered") is None
    ...
    def test_a_credential_value_cannot_be_stored_where_a_key_name_belongs(
        ...
        with pytest.raises(psycopg.errors.CheckViolation, match="credential_ref_is_a_key_name"):
            domain_store.register_source(a_source(credential_ref="sk-live-abc123XYZ"))
    ...
    def test_the_key_name_check_is_a_shape_check_and_says_so(
        ...
        is.
        """
        domain_store.register_source(a_source(credential_ref="COSMA_SRC_DEMO_TOKEN"))
        row = domain_store.read_source("demo")
        assert row is not None and row["credential_ref"] == "COSMA_SRC_DEMO_TOKEN"

    def test_a_source_may_have_no_credential_at_all(self, domain_store: DomainStore) -> None:
        domain_store.register_source(a_source(credential_ref=None))
        row = domain_store.read_source("demo")
    ...
    def test_a_normalizer_source_with_neither_is_accepted(self, domain_store: DomainStore) -> None:
    ...
    def test_an_unknown_kind_is_refused(self, domain_store: DomainStore) -> None:
        with pytest.raises(psycopg.errors.CheckViolation, match="kind_is_known"):
            domain_store.register_source(a_source(kind="scraper"))

    def test_an_unknown_data_class_is_refused(self, domain_store: DomainStore) -> None:
        with pytest.raises(psycopg.errors.CheckViolation, match="data_class_is_known"):
            domain_store.register_source(a_source(data_class="secret"))
...
class TestOnlyAnImporterReadsALocalInput:
    ...
    def test_an_importer_may_hold_an_input_profile(self, domain_store: DomainStore) -> None:
        domain_store.register_source(
            SourceRow(
        ...
        stored = domain_store.read_source("dataset-import")
        assert stored is not None
        assert stored["input_profile"] == {"root": "/tmp/approved", "inputs": {"rows": "a.jsonl"}}

    def test_a_collector_may_not_hold_one(self, domain_store: DomainStore) -> None:
    ...
    def test_a_normalizer_may_not_hold_one(self, domain_store: DomainStore) -> None:
        with pytest.raises(psycopg.errors.CheckViolation, match="only_an_importer_reads"):
            domain_store.register_source(
                SourceRow(
    ...
    def test_an_importer_may_not_be_granted_an_outbound_profile(
        self, domain_store: DomainStore
    ) -> None:
    ...
    def test_an_importer_may_still_need_a_credential(self, domain_store: DomainStore) -> None:
        ...
        assert domain_store.read_source("protected-dataset") is not None


class TestCursor:
    def test_a_source_with_no_cursor_reads_as_none(self, domain_store: DomainStore) -> None:
        domain_store.register_source(a_source())
        assert domain_store.read_cursor("demo") is None

    def test_a_cursor_is_stored_opaquely_and_returned_unchanged(
        self, domain_store: DomainStore, job_store: JobStore
    ) -> None:
        """The platform does not interpret a cursor, so it must survive untouched."""
        domain_store.register_source(a_source())
        _, attempt_id = claim_one(job_store)
        awkward = {"next": None, "page": 3, "seen": ["a", "b"], "nested": {"deep": True}}
        domain_store.advance_cursor("demo", awkward, attempt_id)
        assert domain_store.read_cursor("demo") == awkward
    ...
    def test_advancing_twice_replaces_rather_than_accumulates(
        ...
        domain_store.register_source(a_source())
        _, attempt_id = claim_one(job_store)
        domain_store.advance_cursor("demo", {"page": 1}, attempt_id)
        domain_store.advance_cursor("demo", {"page": 2}, attempt_id)
        assert domain_store.read_cursor("demo") == {"page": 2}
    ...
    def test_streams_of_one_source_do_not_share_a_position(
        ...
        domain_store.register_source(a_source())
        _, attempt_id = claim_one(job_store)
        domain_store.advance_cursor("demo", {"page": 1}, attempt_id, stream="items")
        domain_store.advance_cursor("demo", {"page": 9}, attempt_id, stream="reviews")
        assert domain_store.read_cursor("demo", stream="items") == {"page": 1}
        assert domain_store.read_cursor("demo", stream="reviews") == {"page": 9}
    ...
    def test_a_cursor_records_which_attempt_moved_it(
        ...
        row = job_connection.execute(
            "select updated_by_attempt from cosmai.source_cursor where source_id = 'demo'"
        ).fetchone()
    ...
    def test_a_cursor_cannot_name_an_attempt_that_does_not_exist(
        ...
        with pytest.raises(psycopg.errors.ForeignKeyViolation):
            domain_store.advance_cursor("demo", {"page": 1}, uuid4())
...
class TestRaw:
    def test_an_envelope_records_its_digest_and_provenance(
        ...
        domain_store.register_source(a_source())
        job_id, attempt_id = claim_one(job_store)
        body = b'{"data": [1, 2]}'
        ...
        row = job_connection.execute(
            ...
            "from cosmai.raw_envelope where id = %s",
            (envelope_id,),
        ).fetchone()
        assert row is not None
    ...
    def test_a_payload_that_is_not_utf8_survives_unchanged(
        ...
        row = job_connection.execute(
            "select body from cosmai.raw_envelope where id = %s", (envelope_id,)
        ).fetchone()
    ...
    def test_an_importer_envelope_names_its_input_instead(
        ...
        domain_store.register_source(a_source(kind="importer", outbound_profile=None))
        job_id, attempt_id = claim_one(job_store)
        envelope_id = domain_store.record_envelope(
            "demo", job_id, attempt_id, ADDON_ID, ADDON_VERSION,
        ...
        row = job_connection.execute(
            "select endpoint_ref, input_ref from cosmai.raw_envelope where id = %s", (envelope_id,)
        ).fetchone()
...
class TestSnapshot:
    def test_a_sealed_snapshot_reads_back_in_the_order_it_fixed(
        ...
        snapshot_id = domain_store.seal_snapshot("demo", members)
        read = domain_store.read_snapshot_items(snapshot_id)
        assert [item["item_key"] for item in read] == ["b", "a"]

    def test_a_sealed_snapshot_verifies(self, domain_store: DomainStore) -> None:
        domain_store.register_source(a_source())
        snapshot_id = domain_store.seal_snapshot(
    ...
    def test_an_altered_payload_is_detected(
        self, domain_store: DomainStore, job_connection: psycopg.Connection[Any]
    ) -> None:
        ...
        problems = domain_store.snapshot_tampering(snapshot_id)
    ...
    def test_a_removed_member_is_detected(
        ...
        job_connection.execute(
            "delete from cosmai.snapshot_item where snapshot_id = %s and ordinal = 1",
            (snapshot_id,),
        )
        problems = domain_store.snapshot_tampering(snapshot_id)
        assert any("2 members but 1" in problem for problem in problems)

    def test_a_snapshot_cannot_hold_one_key_twice(self, domain_store: DomainStore) -> None:
    ...
    def test_verification_of_a_snapshot_that_does_not_exist_says_so(
        ...
        missing = uuid4()
        assert domain_store.snapshot_tampering(missing) == (f"snapshot {missing} does not exist",)
...
class TestCollectionIsAtomic:
    ...
    def collect(
        ...
        Returns whether it committed. This is the pattern `apps/domain/store.py`'s module
        docstring specifies and the one B0.3's capability layer must use.
        """
        job_id, attempt_id = claim_one(job_store)
        try:
            with job_connection.transaction():
                ...
                domain_store.advance_cursor("demo", cursor_value, attempt_id)
                if fail_after_writes:
                    raise RuntimeError("interrupted after the writes, before completion")
                completion = job_store.complete_success(job_id, attempt_id, "worker-1")
    ...
    def test_a_committed_collection_leaves_raw_and_cursor_agreeing(
        ...
        domain_store.register_source(a_source())
        assert self.collect(domain_store, job_store, job_connection, {"page": 1}) is True

        assert domain_store.count_items("demo") == 1
        assert domain_store.read_cursor("demo") == {"page": 1}
    ...
    def test_an_interruption_before_completion_leaves_no_raw_and_no_cursor(
        ...
        assert (
            self.collect(
                domain_store, job_store, job_connection, {"page": 1}, fail_after_writes=True
            )
            is False
        )

        assert domain_store.count_items("demo") == 0
        assert domain_store.read_cursor("demo") is None
        row = job_connection.execute("select count(*) from cosmai.raw_envelope").fetchone()
        ...
        state = job_connection.execute("select state from cosmai.job").fetchone()
        assert state is not None and state[0] == JobState.RUNNING.value
        open_attempts = job_connection.execute(
            "select count(*) from cosmai.job_attempt where finished_at is null"
        ).fetchone()
    ...
    def test_the_interruption_test_is_not_passing_because_nothing_was_written(
        ...
        assert self.collect(domain_store, job_store, job_connection, {"page": 1}) is True
        assert domain_store.count_items("demo") == 1
        assert domain_store.read_cursor("demo") is not None
    ...
    def test_a_worker_that_lost_its_lease_persists_neither_raw_nor_cursor(
        ...
        job_connection.execute(
            "update cosmai.job set lease_owner = 'worker-2' where id = %s", (job_id,)
        )

        class LeaseLost(RuntimeError):
            pass

        with pytest.raises(LeaseLost), job_connection.transaction():
            envelope_id = domain_store.record_envelope(
                "demo", job_id, attempt_id, ADDON_ID, ADDON_VERSION,
                body=b'{"data": [1]}', endpoint_ref="items",
            )
    ...
    def test_the_fence_accepts_the_same_sequence_when_the_lease_is_still_held(
        ...
        with job_connection.transaction():
            ...
            domain_store.advance_cursor("demo", {"page": 1}, attempt_id)
            assert job_store.complete_success(job_id, attempt_id, "worker-1")
    ...
    def test_a_second_collection_advances_the_cursor_without_losing_earlier_raw(
        ...
        domain_store.register_source(a_source())
        assert self.collect(domain_store, job_store, job_connection, {"page": 1}) is True
        assert self.collect(domain_store, job_store, job_connection, {"page": 2}) is True
...
class TestTheShapeChecksTheDatabaseHolds:
    ...
    def _envelope(self, domain_store: DomainStore, job_store: JobStore) -> UUID:
        domain_store.register_source(a_source())
        job_id, attempt_id = claim_one(job_store)
    ...
    def test_a_raw_envelope_digest_must_look_like_a_sha256(
        ...
        envelope_id = self._envelope(domain_store, job_store)

        with pytest.raises(psycopg.errors.CheckViolation, match="raw_envelope_digest_is_a_sha256"):
            job_connection.execute(
                "update cosmai.raw_envelope set body_sha256 = %s where id = %s",
    ...
    def test_a_valid_digest_still_updates(
        ...
        job_connection.execute(
            "update cosmai.raw_envelope set body_sha256 = %s where id = %s", ("a" * 64, envelope_id)
        )

    def _snapshot(self, domain_store: DomainStore) -> UUID:
        domain_store.register_source(a_source())
        return domain_store.seal_snapshot(
    ...
    def test_a_snapshot_manifest_digest_must_look_like_a_sha256(
        ...
        snapshot_id = self._snapshot(domain_store)

        with pytest.raises(
            psycopg.errors.CheckViolation, match="snapshot_manifest_digest_is_a_sha256"
        ):
            job_connection.execute(
                "update cosmai.snapshot set manifest_sha256 = %s where id = %s",
                ("nope", snapshot_id),
            )
    ...
    def test_a_snapshot_item_count_cannot_be_negative(
        ...
        with pytest.raises(
            ...
            job_connection.execute(
                "update cosmai.snapshot set item_count = -1 where id = %s", (snapshot_id,)
            )
    ...
    def test_a_snapshot_item_digest_must_look_like_a_sha256(
        ...
        with pytest.raises(
            ...
            job_connection.execute(
                "update cosmai.snapshot_item set payload_sha256 = %s where snapshot_id = %s",
                ("nope", snapshot_id),
            )
    ...
    def test_a_snapshot_item_ordinal_is_zero_based(
        ...
        with pytest.raises(
            ...
            job_connection.execute(
                "update cosmai.snapshot_item set ordinal = -1 where snapshot_id = %s",
                (snapshot_id,),
            )
    ...
    def test_an_attempt_number_is_one_based(
        ...
        with pytest.raises(
            ...
            job_connection.execute(
                "update cosmai.job_attempt set attempt_no = 0 where id = %s", (attempt_id,)
            )
