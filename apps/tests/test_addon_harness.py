...
Copy-adapted from ``experiments/integrated-p0/tests/test_addon_harness.py`` (M3
batch 3a). The one change is where the conformance-normalizer fixture lives: P0
committed ``normalizer.conformance`` under its own ``addons/`` tree, which this
tree reserves for real, M4-installed add-ons (nothing is installed there yet).
It is a test double in the sense P0's own module docstring describes — the
smallest add-on that satisfies the contract, used to exercise the harness and
OQ-003's determinism requirement — so it moved to
``tests/fixtures/normalizer.conformance/`` instead, unchanged otherwise.
...
CONFORMANCE_NORMALIZER = Path(__file__).resolve().parent / "fixtures" / "normalizer.conformance"
