...
opened as (
    ...
    select gen_random_uuid(),
           s.id,
           coalesce(
               (select max(a.attempt_no) from cosmai.job_attempt a where a.job_id = s.id), 0
           ) + 1,
           %(worker_id)s,
           c.correlation_id
    from started s
...
class JobStore:
    ...
    def _attempt_number(self, attempt_id: UUID) -> int:
        ...
        with self._cursor() as cursor:
            cursor.execute(
                "select attempt_no from cosmai.job_attempt where id = %(attempt_id)s",
                {"attempt_id": attempt_id},
            )
            row = cursor.fetchone()
    ...
    def _correlation_of(self, job_id: UUID) -> str | None:
        ...
        with self._cursor() as cursor:
            cursor.execute(
                "select correlation_id from cosmai.job where id = %(job_id)s", {"job_id": job_id}
            )
            row = cursor.fetchone()
