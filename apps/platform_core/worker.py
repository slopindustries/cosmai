...
Copy-adapted from ``experiments/integrated-p0/platform_core/worker.py``. The
only substantive change is ``describe()``'s call shape: DP-032 gives
``platform_core.db.connection.describe`` a second, required ``role`` argument
(P0-A connected under one implicit identity; DP-032 D1 splits ``runtime`` from
``migrator``), so the classification call below states ``role="runtime"`` —
the only role a worker process ever opens. Nothing else about lifecycle,
shutdown, or the report changed; DP-032 moved where the database lives, not
what a worker does once it can reach one.
...
#: The identity a worker's own connection always opens under. DP-032 D1 splits
#: ``runtime`` from ``migrator``; a worker never does DDL, so this is the only
#: role its connection — and therefore its failure classification — ever names.
_ROLE: Final = "runtime"
...
def parse_arguments(argv: Sequence[str] | None = None) -> WorkerOptions:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Run the P1 platform worker: claim jobs, execute their handlers, "
