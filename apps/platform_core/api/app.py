...
Copy-adapted from ``experiments/integrated-p0/platform_core/api/app.py``. The
one structural change is the connection helper: P0-A's
``platform_core.db.connection`` exported a ``connected(config, ...)`` context
manager alongside ``connect``; DP-032's ``platform_core.db.connection`` (Task 4)
exports only ``connect``, which — being an ordinary ``psycopg.Connection`` —
already closes itself on the way out of a ``with`` block (see
``psycopg.Connection.__exit__``), so every ``with connected(config, ...)`` below
became ``with connect(config, ...)`` with no other change. Nothing about what an
endpoint does, reads, or redacts is different from P0-A.
