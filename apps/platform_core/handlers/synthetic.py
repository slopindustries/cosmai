"""The failure injectors the worker's scenario evidence needs, and nothing that resembles work.

Copy-adapted from ``experiments/integrated-p0/platform_core/handlers/synthetic.py``.
Nothing here changed: DP-032 moved where the database lives, not what
CONTRACT-JOB@0.1's state machine does once a job reaches it, so the injectors
that exercised that machine in P0-A exercise the same machine here, unmodified.
...
def synthetic_registry() -> HandlerRegistry:
    """A registry holding exactly the handlers above. The worker's whole table."""
