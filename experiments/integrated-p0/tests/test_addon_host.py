...
#: ``experiments/integrated-p0``, where ``addons/`` sits by default.
...
#: given: B0.1 builds no context, so there is nothing kind-shaped to accept yet.
ENTRY_SOURCE = """
def run(context):
    return None
"""
...
def _no_effect(key: str, value: Any) -> bool:
    raise AssertionError("no B0.1 test applies a durable effect")
...
def call_entry(addon: LoadedAddon, context: JobContext) -> None:
    """Stand-in for the capability layer B0.3 owns.

    It calls the entry point with the ``JobContext`` rather than the kind's own
    context, because building that context needs ``domain``. Enough to prove the
    call path and the translation around it; not a capability.
    """
    addon.entry(context)
...
def test_a_template_directory_is_not_installed(tmp_path: Path) -> None:
    """B0.4 puts a template at ``addons/_template/``; a template is not installed."""
...
def test_the_gate_is_evaluated_against_the_contract_version_it_is_given(
    ...
    editing ``addon_api``, which this work package may not do.
    """
...
def test_a_handler_name_is_namespaced_so_an_add_on_cannot_shadow_the_platform(
    ...
    """``HandlerRegistry.register`` refuses a rebind, so a collision is fatal."""
    registry = synthetic_registry()
...
def test_an_add_on_that_is_not_installed_still_fails_as_handler_unknown(
    ...
    """Contracted P0-A behaviour, which is why B0.1 builds nothing for the absent case."""
    registry = HandlerRegistry()
    install_addons(registry, root=tmp_path)
...
def test_the_default_root_is_the_in_repository_addons_directory() -> None:
    ...
    assert DEFAULT_ADDON_DIR == EXPERIMENT_ROOT / "addons"
