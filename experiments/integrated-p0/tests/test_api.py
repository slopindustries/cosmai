...
Both scenarios say in as many words that a document's claim is not evidence.
...
**The probe collects, then the tests assert.** ``redaction_probe`` runs the whole
of SEC-004's Action — create the marked job, fail it permanently, read all four
representations, stop the API — and freezes the results. Each test is then a
read-only assertion over the same set of artefacts, which is both what makes the
"searched every artefact" claim checkable in one place and what stops five tests
replaying one four-second setup.

**Every marker is injected to be found.** The payload carries a distinctive value
under every key in the contract's redaction set *and* one under an ordinary key.
The ordinary one is the detection control: if it were also missing, "no marker was
found" would be indistinguishable from "the search does not work".

**The platform does not log payloads, so the log search needs its own control.**
Searching the worker's log for a marker that the platform was never going to write
there is a weak assertion on its own. ``test_sec_004_the_structured_log_masks_a
_payload_it_is_handed`` supplies the missing half by handing the marked payload
straight to a logger and observing what comes out.
"""
...
#: Long enough to let an interpreter start; a process that was supposed to refuse
#: its configuration and is still alive after this is the failure, not the wait.
...
def refused_api(config: PlatformConfig, **overrides: str) -> subprocess.CompletedProcess[str]:
    ...
    Not ``running_api``: that one waits for a listening socket, and the whole point
    of these cases is that no socket is ever bound.
    """
...
def non_loopback_addresses() -> list[str]:
    ...
    Two sources, because neither is complete on its own: the host's own name may
    resolve to nothing useful, and the routing table knows only the address that
    would be used to leave the machine. The union is what SEC-002 step 3 asks to be
    enumerated. No packet is sent — a connected UDP socket only picks a route.
    """
    found: set[str] = set()
    try:
...
def listening_tcp_sockets() -> str:
    """The host's listening TCP sockets, as recorded output rather than a summary."""
    tool = which("lsof")
    if tool is None:  # pragma: no cover - depends on the host
        pytest.skip("lsof is not on PATH, so listening sockets cannot be enumerated")
    result = subprocess.run(
        [tool, "-nP", "-iTCP", "-sTCP:LISTEN"],
        capture_output=True,
        text=True,
        check=False,
    )
    # lsof exits non-zero when it merely could not read some other user's process.
    assert result.stdout, f"lsof produced no output: {result.stderr}"
...
def test_sec_002_the_api_binds_a_loopback_address_and_records_the_one_it_got(
    database: PlatformConfig, host: str
) -> None:
    ...
    """
    with running_api(database, host=host) as api:
...
def test_sec_002_the_api_is_unreachable_from_every_non_loopback_address_of_the_host(
    ...
    """Step 3, over every address of this host that is not loopback."""
    ...
    with running_api(database) as api:
...
def test_sec_002_the_api_entrypoint_refuses_a_non_loopback_bind(
    ...
    finished = refused_api(database, COSMA_API_HOST=host)
...
def test_sec_002_the_wildcard_bind_is_not_quietly_replaced_by_loopback(
    ...
    finished = refused_api(database, COSMA_API_HOST="0.0.0.0", COSMA_API_PORT=str(port))
    assert finished.returncode == EXIT_CONFIGURATION_INVALID, finished.stderr
    with pytest.raises(OSError):
...
def test_sec_002_the_two_entrypoints_use_the_same_exit_statuses() -> None:
    ...
    Two spellings of one closed set drift, and a supervisor that learned to tell
    ``78`` from ``1`` on the worker would silently mis-read the API.
    """
...
def test_sec_002_no_postgresql_tcp_listener_exists_on_this_host() -> None:
    """Step 4. DP-006 D2 removed the listener rather than binding it locally."""
    recorded = listening_tcp_sockets()
    offending = [
        line
        for line in recorded.splitlines()
        if line.split(" ")[0].lower().startswith(("postgres", "postmaster"))
    ]
    assert offending == [], recorded


def test_sec_002_the_database_session_is_not_a_tcp_connection(
    db_connection: psycopg.Connection[Any],
) -> None:
    """Step 5. ``inet_server_addr()`` is null only over a Unix socket."""
    with db_connection.cursor() as cursor:
        cursor.execute("select inet_server_addr() is null, inet_server_port() is null")
        row = cursor.fetchone()
        assert row == (True, True), row
        cursor.execute("show listen_addresses")
        listening = cursor.fetchone()
    assert listening is not None
    assert listening[0] == "", f"the cluster advertises a TCP listener: {listening[0]!r}"
...
def test_an_unknown_job_is_a_404(database: PlatformConfig) -> None:
    absent = uuid4()
    with running_api(database) as api:
...
def test_sec_003_case_f_the_api_entrypoint_reports_an_unknown_variable_and_runs(
    ...
    with running_api(database, **{UNRELATED_NAME: UNRELATED_VALUE}) as api:
        assert (
...
class RedactionProbe:
    ...
    def job_artefacts(self) -> dict[str, str]:
        ...
        The worker's metric report is excluded: it is a per-process reading with no
        job context, so requiring a correlation identifier in it would be asserting
        a shape CONTRACT-JOB@0.1 does not give it.
        """
        return {
...
) -> Iterator[RedactionProbe]:
    """SEC-004's Action, steps 1 to 3, with every artefact frozen for inspection.

    Module-scoped and therefore owns its own database. Every test that reads it
    asserts over the frozen ``RedactionProbe`` and touches nothing else, so one
    execution serves all of them; at function scope the same worker run and API
    start were replayed twenty-two times for twenty-two read-only assertions.
    """
    keep = keep_databases(request)
    with cloned_database(platform_database, migrated_template, "test", keep) as database:
        quiet = StructuredLogger(stream=StringIO(), level="DEBUG")
        with connected(database, autocommit=True) as handle:
            store = JobStore(handle, database, logger=quiet, metrics=MetricsRegistry())
        ...
        finished = run_worker(database, "--once")
...
def test_sec_004_protected_does_not_mean_unredacted(store: JobStore) -> None:
    """The clause the probe above cannot reach, with its own detection control.

    ``fail_permanent`` builds its own detail, so the markers in that job's payload
    never enter ``error_detail`` and the probe's protected representation is
    therefore only evidence that detail is *reachable*. Here the detail itself
    carries a marker under every redacted key, written through the same completion
    path a handler uses, and the protected representation is read back from the
    stored row. The ordinary marker in the same mapping survives, which is what
    makes the masking an observation rather than an absence.
    """
    job_id = store.create_job("succeed", {}, max_attempts=MAX_ATTEMPTS)
    ...
    completion = store.complete_permanent(
        ...
        claimed.attempt_id,
        WORKER,
        PlatformPermanentError("the permanent failure injector refused this attempt", {
            **MARKED_PAYLOAD,
    ...
    rows = store.read_attempts(job_id)
    assert len(rows) == 1
...
def test_sec_004_the_structured_log_masks_a_payload_it_is_handed() -> None:
    ...
    The platform never writes a payload into an event, so searching a real log for
    a marker is an assertion about what was not attempted. This attempts it.
    """
    stream = StringIO()
