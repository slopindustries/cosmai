...
Captured by `-k sec_004 --capture-evidence=DIR`. The assertions in
test_dashboard.py are the authority; this file is what they were asserted over.
...
def test_sec_004_the_screens_are_written_to_the_evidence_directory(
    screen_probe: ScreenProbe, request: pytest.FixtureRequest
) -> None:
    """Write the rendered screens where the gate reviewer can read them, when asked.

    SEC-004's Verification names an evidence location, and a screen asserted over but
    never captured leaves the reviewer taking the assertions on trust. Capture is
    opt-in through ``--capture-evidence=DIR``: writing into the committed directory on
    every run made the hashes recorded there unverifiable, which is worse than no
    capture because it looks like tampering.
    """
    directory = capture_directory(request)
    if directory is None:
        pytest.skip(f"capture not requested; pass {CAPTURE_OPTION}=DIR")
    target = directory / "sec-004-detail-screen.txt"
