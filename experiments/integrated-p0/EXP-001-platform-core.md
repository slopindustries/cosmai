...
```text
[측정] The redaction boundary holds on the screen an operator actually reads.
```

- procedure: `SEC-004`'s Action carried through to the dashboard. Three jobs — a marked payload failed by a real worker, a job whose `error_detail` carries its own markers, and a job whose retry is refused — rendered through the same component tree the browser mounts, server-side, from responses obtained from a real API process. Five screens, searched for every marker.
- observed: 60 passed. For each of the eight redacted keys, its marker appears on **no** screen, while the redaction marker appears where the value was removed and the key name survives. The marker under an ordinary key **is** on the screen — the detection control. `error_detail` is absent from the default screen and present on the protected one, and reserved keys stay masked even there. The correlation identifier is present and unmasked. A refused retry shows both the current and the required state.
- captured_at: 2026-08-17
- limitation: this is step 3, the reading half. **Step 4's screenshot is not executed** — it needs a browser driver and DP-006 D6 puts the dependency floor below one. A value hidden by CSS or parked in a DOM attribute would pass a text search and fail a person's eyes. The captured screens are in the evidence directory as `sec-004-detail-screen.txt`, regenerated by `-k sec_004` so they cannot drift from what was asserted.

```text
[측정] The markup a browser receives is now searched, and the search catches what a
screenshot would not.
```

- procedure: `renderJobDetailMarkup` exposes the undecorated `renderToStaticMarkup` output the renderer was already computing and discarding; `-k sec_004` now searches both halves. Verified by planting leaks and observing which assertions fire.
- observed: 72 passed, up from 59. A withheld value rendered into a `data-` attribute — invisible to a reader, delivered to the browser — failed four assertions including the new value-based one, with the message naming the marker. Reverted; `git diff` empty.
- captured_at: 2026-08-17
- limitation: the remaining uncovered channel is a value rendered into an **image**. P0-A renders none, so that set is empty rather than narrow.

```text
[추론] The screenshot I originally required would have been the weaker artifact.
```

`SEC-004` protects against a marked value reaching a surface where a reader can recover it. The browser receives markup; a value hidden by CSS or parked in an attribute is **absent from a screenshot and present in what was delivered**, recoverable by view-source. So the artifact the first gate draft demanded would have missed the leak it claimed to catch, while the artifact that catches it was already being computed and thrown away — `detail-text.tsx`'s own comment said so ("a marker that leaked into a `title=` would not be found") and I prescribed a screenshot anyway. The charter's word "screenshots" concerns screenshots as evidence artifacts; P0-A produces none, so the obligation binds P0-B, which will capture domain screens.

```text
[측정] A leak at the API's own withholding point left the API but never reached a screen.
```

- procedure: `attempt_view` was made to ship the withheld `error_detail` under an innocuous `debug_hint` key. `attempt_view(row, protected=False)` was called directly to confirm the marker was in the response.
- observed: the marker **was** in the API response, and all 71 SEC-004 assertions still passed. It never reached the markup: the dashboard renders named fields and no unknown key, so the value stopped at that boundary.
- captured_at: 2026-08-17
- limitation: this is a property of the current components, not an enforced invariant. A future component that spreads an attempt object would remove it.

```text
[추론] Two of my own controls were weaker than I described them, in opposite directions.
```

The screenshot requirement asked for less than the markup already available. The log requirement asked for something that already existed — `test_sec_004_the_structured_log_masks_a_payload_it_is_handed` hands the logger a marked payload and asserts the ordinary marker survives and the redaction count is exact, which is a positive control. Two facts hold there: the logger redacts what it is handed, and the platform never hands it a payload. Together they are stronger than "the log was searched", and the gate's first draft claimed the search as the evidence, which understated it and invited the reviewer's F7.

`[추론]` The value-based assertion is the durable part of this. Every other assertion names the field it inspects, and the `debug_hint` plant showed what that lets through: a key nobody thought to exclude is invisible to a field-named check. Searching by value closes the class rather than the instance.

```text
[측정] The dashboard contains no forbidden vocabulary, with no parser enforcing it.
```

- procedure: `grep -rniE 'raw|records|observation|snapshot|normaliz|collector|importer|lineage|provider' experiments/integrated-p0/dashboard/src/`
- observed: no matches. The screen labels its payload panel "payload", not "raw payload".
- captured_at: 2026-08-17
- limitation: this is one grep at one moment, not a gate. The boundary guard parses `.py` and `.sql` only, so `.ts` and `.tsx` are checked by file name alone.

```text
[추론] Naming the substitutions in advance was cheaper than extending the guard.
```

The guard cannot read TypeScript, and extending it would mean a Python-side regex scanner treating `//`, `/* */`, JSDoc, template literals, and JSX text as prose. That is the same trap the SQL detector in that guard already fell into once, when the English word "from" in a docstring made it parse prose as a statement. So the exposure was narrowed instead: the substitutions were fixed in the task brief before the code existed — `payload` not `raw payload`, `rows` not `records` — and data-shape types were derived from API response fields rather than invented, which leaves no route for domain vocabulary to enter. `raw` is the standard idiom of a debug view, so this was a real risk rather than a hypothetical one, and the result is zero occurrences with nothing mechanical stopping them.

The gap is real and stays recorded: a future contributor who does not read `dashboard/README.md` gets no warning from a test. The gate must carry this as a known limitation rather than as covered.
