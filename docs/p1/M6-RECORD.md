...
| 6 | `docs/decisions/DP-033-p1-scheduler-and-export.md` D3: "Normalized results export as flattened CSV." | `apps/domain/export.py`'s `_result_csv_rows` ships metadata columns (`id`, `snapshot_id`, `source_id`, `addon_id`, `addon_version`, `output_contract_version`, `source_item_key`, `body_sha256`, `created_at`) plus one `json.dumps` `notes` column and one `json.dumps` `body` blob column — `body`'s own fields are not spread into their own CSV columns. | `[정정, 2026-08-21, m7-fixwave, M-X1]` Registered here per B6's/this ledger's own instinct rather than left unnoted (M-X1, `docs/agent-workflow/reviews/REVIEW-M2-M7.md`) — no code change accompanies this row. "Flattened" is exactly the shape the raw-CSV branch's own metadata-plus-one-blob-column design already uses and this ledger's row 1 already registers as a deliberate superset relationship between the two export kinds; this row makes the *body*-column shape itself an explicit, named deviation from D3's literal text rather than an implicit one a reader would have to notice by reading the code. Whether `body`'s own keys should become their own dynamic CSV columns (which would need a header computed per-export, since different normalizer versions produce different keys) is an open design question this record does not resolve. |
| 7 | — (registration, not a deviation from a stated plan) | `apps/domain/api.py:566` and `apps/domain/export.py`'s result egress paths key-redact the normalized `body` on every read, while `body_sha256` beside it digests the **unredacted** body at write time. | `[등록, 2026-08-21, m7-fixwave, M-S6]` Both call sites already document this in their own comments; no record or Decision Packet had. The practical consequence: an operator who exports a redacted result and tries to verify `body_sha256` against the exported `body` cannot — the hash was computed over content the export no longer shows. Not a security defect (redaction is working as intended); a verification-utility gap, registered per M-S6, `docs/agent-workflow/reviews/REVIEW-M2-M7.md`. No code change accompanies this row. |
