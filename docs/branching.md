...
`main`으로의 병합은 PR을 거친다.

`[확인 사실]` **이것은 강제되지 않는다.** 비공개 저장소의 브랜치 보호는 GitHub Pro를
요구하고, 2026-08-18에 시도했다가 `403 Upgrade to GitHub Pro`로 거부됐다. 계정은
`ADMIN` 권한을 갖고 있으므로 권한 문제가 아니라 요금제 제약이다. 자세한 기록은
[EXP-002](../experiments/integrated-p0/EXP-002-addon-layer.md)에 있다.

`[결정]` 로컬 `pre-push` hook은 일부러 설치하지 않았다. `--no-verify`로 우회되고 이
체크아웃에서만 유효하므로 **보호처럼 보이면서 보호가 아니다.** 실제보다 강해 보이는
통제는 기록된 부재보다 나쁘다.

그래서 이 절은 규칙이 아니라 **관례**로 읽어야 한다. 지키는 것은 사람과 에이전트이지
서버가 아니다.
...
`[확인 사실]` `origin`에 이 작업과 무관한 브랜치가 있다:
`agent/document-agent-operating-model`, `sooho`. 위 규칙 이전의 것이며 여기서 다루지
않는다. 기준점은 `dev`다.
