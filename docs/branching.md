...
`[가설]` **제안 상태이며 수락되지 않았다.** 이 예외는 소유자에게 묻지 않고 쓰였고,
[OQ-011](open-questions/OQ-011-agent-memory-and-area-boundary.md) R2가 이것을 예외로 남길지,
여섯 번째 영역으로 만들지, 철회할지를 묻는다. **이 예외의 예시가 이 예외를 도입한 브랜치라는 점**이
합의를 추정하기 가장 어려운 이유다.

**개발 영역에 속하지 않는 작업이 하나 있다.** 프로젝트 운영 방식 자체를
